...
可以尝试着使用`btrfs balance start -dusage=0 /`来把使用率为0%的数据块组整理一下, 注意, `dusage=0`表示使用率为0%的数据块组, `dusage=X`(`X`不为0)表示小于等于`X`的数据块组, 如`btrfs balance start -dusage=1 /`, 表示平衡小于等于1%的数据块组. 
