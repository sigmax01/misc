...
2. 生成CA的公钥和私钥
    ```
    openssl genrsa -out /Users/wenzexu/man/frp/ca-client.key 2048
    openssl req -x509 -new -nodes -key /Users/wenzexu/man/frp/ca-client.key -subj "/CN=example.ca.com" -days 5000 -out /Users/wenzexu/man/frp/ca-client.crt
    ```
3. 生成一个客户端的私钥
    ```
    openssl genrsa -out /Users/wenzexu/man/frp/client.key 2048 
    ```
4. 生成一个CSR数字签名请求
    ```
    ...
    -reqexts SAN \
    -config <(cat my-openssl.cnf <(printf "\n[SAN]\nsubjectAltName=DNS:localhost,IP:127.0.0.1,DNS:example.client.com")) \
...
5. 使用CSR向CA发起签名请求, 并返回客户端的公钥
    ...
    -extfile <(printf "subjectAltName=DNS:localhost,IP:127.0.0.1,DNS:example.client.com") \
